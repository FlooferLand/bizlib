...
public partial class Main {
    ...
    public static unsafe RshwData ReadRshw(IntPtr handle, int size) {
        if (handle == IntPtr.Zero) return RshwData.Error("BizlibNative received a null pointer instead of a show");
    ...
    public static RshwData ReadRshwFile(IntPtr pathPtr) {
        ...
        if (string.IsNullOrEmpty(path) || !File.Exists(path)) return RshwData.Error("File does not exist");
    ...
    public static void FreeRshw(RshwData data) {
        ...
        if (data.errorPtr != IntPtr.Zero) Marshal.FreeHGlobal(data.errorPtr);
    }
}